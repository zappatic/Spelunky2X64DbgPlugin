...
ViewState::ViewState(State* state, EntityDB* entityDB, QWidget* parent) : QWidget(parent), mState(state), mEntityDB(entityDB)
{
...
void ViewState::refreshState()
{
    ...
    {
        mMainTreeView->updateValueForField(field, field.name, mState->offsets());
    }
}
